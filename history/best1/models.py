...
class BERT_CNN(nn.Module):
    def __init__(self, bert_model_name='google-bert/bert-base-multilingual-uncased', input_dim=768, num_filters=256,
                 kernel_sizes=[3, 4, 5], num_classes=2, dropout_rate=0.5):
    ...
    def forward(self, input_ids, attention_mask=None):
        ...
        # Debug: Check shapes after each conv layer and pooling
        # for i, conv_out in enumerate(conv_outs):
        #     print(f"conv_out shape after conv {i+1}: {conv_out.shape}")

        x = torch.cat(conv_outs, 1)
        # Debug: Check x shape after concatenation
